...
新人が最初に迷うのは「結局、どの順で何を埋めればよいか」であるため、章を横断する通し例を1本示す。

注意: ここでの `libraryX` やURLは例示（ダミー）であり、実在の製品/組織に合わせて読み替える。
...
- デフォルトは10s。`CLIENT_TIMEOUT` で変更可能。接続確立前と確立後で扱いが異なる（例外あり）
